...
import com.example.test.Reponsitory.*;
...
//                System.out.println("oiddddddddđ" + order.getId());
            ...
            redirectAttributes.addFlashAttribute("error", "Thông tin đơn hàng khng hợp lệ, vui lòng nhập lại!");
        }
...
//        model.addAttribute("itemsWithFineList", itemIssuanceService.getItemsWithFine());
    ...
    public String addToCart(Model model, @RequestParam Integer id) {
        ...
        shoppingCartService.add(new CartItem(product.getId(), product.getName(), product.getPhoto(), 1, product.getPrice()));
//        model.addAttribute("itemsWithFineList", itemIssuanceService.getItemsWithFine());
    ...
    @GetMapping("/history")
    public String hítory(Model model) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        UserDetails userDetail = (UserDetails) authentication.getPrincipal();
        System.out.println(userDetail.getAuthorities());
        try {
